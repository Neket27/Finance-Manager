...
public class App {
    ...
    public static void main(String[] args) {
        ...
        FinanceRepository financeRepository = new FinanceJdbcRepository(dbConfig.getConnection());
        FinanceMapper financeMapper = new FinanceMapper();
        TransactionMapper transactionMapper = new TransactionMapper();
        UserService userService = new UserServiceImpl(new UserMapper(), new UserJdbcRepository(dbConfig.getConnection()), financeRepository, financeMapper);
        AuthService authService = new AuthServiceImpl(authenticationConfig, authenticator, userService);
        TransactionService transactionService = new TransactionServiceImpl(new TransactionJdbcRepository(dbConfig.getConnection()), transactionMapper);
        ...
        LiquibaseConfig liquibaseConfig = new LiquibaseConfig(dbConfig);
